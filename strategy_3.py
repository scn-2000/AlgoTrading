...
#Importing tickers
df = pd.read_excel('tickers-bt06-wilshire5000-plus5bn-v6-sn69.xlsx')
...
#START AVERAGE OF LAST 4 RSI(14) BELOW 49
#START AVERAGE OF LAST 4 RSI(14) BELOW 49
#START AVERAGE OF LAST 4 RSI(14) BELOW 49
...
for i in rsi_df:
    indicator_sma_rsi_df = SMAIndicator(close=rsi_df[i], window=3)
    sma_of_rsi_df[i] = indicator_sma_rsi_df.sma_indicator()

sma_of_rsi_df = sma_of_rsi_df.mask(sma_of_rsi_df<49, 1)
sma_of_rsi_df = sma_of_rsi_df.mask(sma_of_rsi_df>49, 0)
...
stoch_rsi_avg = stoch_rsi_avg.mask(stoch_rsi_avg<=23, 200)
...
#START RSI(14) NOT ABOVE 72 IN PAST 11 TRADING DAYS
#START RSI(14) NOT ABOVE 72 IN PAST 11 TRADING DAYS
#START RSI(14) NOT ABOVE 72 IN PAST 11 TRADING DAYS
...
rsi_df = rsi_df.mask(rsi_df<72, 0)
rsi_df = rsi_df.mask(rsi_df>=72, 1)

#Creating the SMA(11) on the RSI(14)
list5 = []
rsi_above_72_df = pd.DataFrame(list5)

for i in rsi_df:
    indicator_sma_rsi_df = SMAIndicator(close=rsi_df[i], window=11)
    rsi_above_72_df[i] = indicator_sma_rsi_df.sma_indicator()

rsi_above_72_df = rsi_above_72_df.mask(rsi_above_72_df>0.08, 50)
rsi_above_72_df = rsi_above_72_df.mask(rsi_above_72_df<0.08, 1)
rsi_above_72_df = rsi_above_72_df.mask(rsi_above_72_df>45, 0)

#END RSI(14) NOT ABOVE 72 IN PAST 11 TRADING DAYS
#END RSI(14) NOT ABOVE 72 IN PAST 11 TRADING DAYS
#END RSI(14) NOT ABOVE 72 IN PAST 11 TRADING DAYS



#START RSI(14) ON DAY OF ENGULF < 56
#START RSI(14) ON DAY OF ENGULF < 56
#START RSI(14) ON DAY OF ENGULF < 56
list4 = []
rsi_below55_df = pd.DataFrame(list4)

for i in adj_close_df:
    indicator_rsi_df = RSIIndicator(close=close_df[i], window=14)
    rsi_below55_df[i] = indicator_rsi_df.rsi()

rsi_below55_df = rsi_below55_df.mask(rsi_below55_df<=56, 1)
rsi_below55_df = rsi_below55_df.mask(rsi_below55_df>56, 0)
#END RSI(14) ON DAY OF ENGULF < 56
#END RSI(14) ON DAY OF ENGULF < 56
#END RSI(14) ON DAY OF ENGULF < 56

buy_sig_df = engulf_sig_df * sma_of_rsi_df * diff_open_15std_df * stoch_rsi_avg * macd_avg * rsi_above_72_df * rsi_below55_df
...
for i in adj_close_df:
    indicator_bb = BollingerBands(close=adj_close_df[i], window=20, window_dev=2.25)
...
rsi_df = rsi_df.mask(rsi_df>=64.25, 102)
